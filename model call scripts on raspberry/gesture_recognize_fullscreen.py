# USAGE
# python gesture_recognize_fullscreen.py --graph graphs/GestureRecognize_6types_iter156000_graph --display 1
...
while True:
    try:
        ...
        if args["display"] > 0:
            ...
            image_for_result = cv2.resize(image_for_result, (0, 0), fx=1.125, fy=1,
                                          interpolation=cv2.INTER_NEAREST)
            cv2.rectangle(image_for_result, (0, int(image_for_result.shape[0] * 0.92)),
                          (image_for_result.shape[1], image_for_result.shape[0]), (255, 255, 255), -1)
            label = "{}: {:.1f}".format("fps", realtime_fps)
            cv2.putText(image_for_result, label, (5, int(image_for_result.shape[0] * 0.972)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            cv2.putText(image_for_result, "temperature: 30", (90, int(image_for_result.shape[0] * 0.972)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            cv2.putText(image_for_result, "lastgesture: 5", (240, int(image_for_result.shape[0] * 0.972)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
